// -*- C++ -*-
//
// Package:    TelescopeDigiGeometryAnalyzer
// Class:      TelescopeDigiGeometryAnalyzer
// 
/**\class TelescopeDigiGeometryAnalyzer TelescopeDigiGeometryAnalyzer.cc 
...
class TelescopeDigiGeometryAnalyzer : public edm::one::EDAnalyzer<>
{
public:
      explicit TelescopeDigiGeometryAnalyzer( const edm::ParameterSet& );
      ~TelescopeDigiGeometryAnalyzer() override;
...
TelescopeDigiGeometryAnalyzer::TelescopeDigiGeometryAnalyzer( const edm::ParameterSet& iConfig )
{}

TelescopeDigiGeometryAnalyzer::~TelescopeDigiGeometryAnalyzer()
{}

// ------------ method called to produce the data  ------------
void
TelescopeDigiGeometryAnalyzer::analyze( const edm::Event& iEvent, const edm::EventSetup& iSetup )
{

   PRINT("TelescopeDigiGeometryAnalyzer")<< "Here I am";
   //
   // get the TelescopeGeom
   //
   edm::ESHandle<TelescopeGeometry> pDD;
   iSetup.get<TelescopeDigiGeometryRecord>().get( pDD );     
   PRINT("TelescopeDigiGeometryAnalyzer")<< " Geometry node for TelescopeGeom is  "<<&(*pDD) <<'\n';   
   PRINT("TelescopeDigiGeometryAnalyzer")<<" I have "<<pDD->detUnits().size() <<" detectors"<<'\n';
   PRINT("TelescopeDigiGeometryAnalyzer")<<" I have "<<pDD->detTypes().size() <<" types"<<'\n';
   ...
   for(auto const & it : pDD->detUnits()){
	...
	PRINT("TelescopeDigiGeometryAnalyzer") << it->geographicalId()
   ...
   for (auto const & it  :pDD->detTypes() ){
     if (dynamic_cast<const PixelGeomDetType*>((it))!=nullptr){
       ...
       PRINT("TelescopeDigiGeometryAnalyzer")<<" PIXEL Det " // << it->geographicalId()
                      <<"    Rows    "<<p.nrows() <<"    Columns "<<p.ncolumns()<<'\n';
     }else{
       const StripTopology& p = (dynamic_cast<const StripGeomDetType*>((it)))->specificTopology();
       PRINT("TelescopeDigiGeometryAnalyzer") <<" STRIP Det " // << it->geographicalId()
                                            <<"    Strips    "<<p.nstrips()<<'\n';
     }
   }
}
void TelescopeDigiGeometryAnalyzer::analyseTrapezoidal( const GeomDetUnit& det)
{
...
void TelescopeDigiGeometryAnalyzer::checkRotation( const GeomDetUnit& det)
{
...
void TelescopeDigiGeometryAnalyzer::checkTopology( const GeomDetUnit& det)
{
...
std::ostream& TelescopeDigiGeometryAnalyzer::cylindrical( std::ostream& os, 
...
DEFINE_FWK_MODULE(TelescopeDigiGeometryAnalyzer);
