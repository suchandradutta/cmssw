...
private:
  ...
  void checkRate();
  int matchEGWithGenParticle(float& pt, float&  eta, float& phi);
...
void L1TkElectronTrackObjectsAnalyzer::checkEfficiency() {
  ...
  float genEta;
  float genPhi;
  int nSelectedEG = matchEGWithGenParticle(genPt, genEta, genPhi);
...
int L1TkElectronTrackObjectsAnalyzer::matchEGWithGenParticle(float& pt, float & eta, float& phi) {
  int indx = 0;
  ...
  const reco::Candidate & p = (*genParticleHandle)[indx];
  pt = p.pt();
  eta = p.eta();
  phi = p.phi();
...
void L1TkElectronTrackObjectsAnalyzer::scaleHistogram(TH1F* th, float fac){
...
int L1TkElectronTrackObjectsAnalyzer::getMotherParticleId(const reco::Candidate& gp) {
  int mid = -1;
