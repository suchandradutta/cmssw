...
TrackTriggerStudy::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  eventBr_->genParticleEt   = (*genParticleHandle)[0].pt();
  eventBr_->genParticleEta  = (*genParticleHandle)[0].eta();
  eventBr_->genParticlePhi = (*genParticleHandle)[0].phi();
  //////////////////////////////////////////////////////////
  // Geometry 
  //////////////////////////////////////////////////////////
  ...
  // TTStubs
  iEvent.getByLabel("TTStubsFromPixelDigis", "StubAccepted", stubHandle);
...
int TrackTriggerStudy::getSimTrkIndex(stubIter s) {
  int index = -5;
...
double TrackTriggerStudy::getZ0(stubIter s1, stubIter s2) {
...
double TrackTriggerStudy::getAngle(stubIter s1, stubIter s2, double z0) {
...
bool TrackTriggerStudy::goodTwoPointZ(double innerR, double outerR, double innerZ, double outerZ ) {
...
bool TrackTriggerStudy::goodTwoPointPhi(double innerR, double outerR, double innerPhi, double outerPhi, double m_strength) {
...
double TrackTriggerStudy::getDPhi(GlobalPoint epos, double eet, double r, double phi, double m_strength) {
...
double TrackTriggerStudy::getZIntercept(GlobalPoint epos, double r, double z) {
...
double TrackTriggerStudy::getTrkEGamdP(GlobalPoint epos, L1TTTrackCollection::const_iterator trk) {
  double er = epos.perp();
...
double TrackTriggerStudy::getTrkEGamOriginZ(GlobalPoint epos, L1TTTrackCollection::const_iterator trk) {
...
double TrackTriggerStudy::getTrkEGamDeltaR(GlobalPoint epos, L1TTTrackCollection::const_iterator trk){
...
double TrackTriggerStudy::getTrkEGamDeltaRCorr(GlobalPoint epos, stubIter s1, stubIter s2, L1TTTrackCollection::const_iterator trk){
  double er = epos.perp();
  double ez = epos.z();
...
double TrackTriggerStudy::getPhiMiss(double eet, GlobalPoint spos1, GlobalPoint spos2) {
...
// ZMiss
double TrackTriggerStudy::getZMiss(GlobalPoint epos, double r1, double r2, double z1, double z2, bool bar) {
...
double TrackTriggerStudy::getScaledZInterceptCut(unsigned int layer, double cut, double cfac, double eta) {
  double mult = (rcal_-radii_[layer])/(rcal_-radii_[1]);
  return (mult*(cut+cfac*(1.0/(1.0-cos(2*atan(exp(-1.0*fabs(eta))))))));
}
// Z-issCut
double TrackTriggerStudy::getScaledZMissCut(int layer1, int layer2, double cut, double cfac, double eta) {
  double mult = ( (radii_[layer2] - radii_[layer1])/(rcal_-radii_[layer1]) )*( (rcal_ -radii_[1])/(radii_[2]-radii_[1]));
  return (mult*(cut+cfac*(1.0/(1.0-cos(2*atan(exp(-1.0*fabs(eta))))))));
}
unsigned int TrackTriggerStudy::getLayerId(StackedTrackerDetId id) {
...
void TrackTriggerStudy::fillRecTrackInfo() {
  ...
  for(L1TTTrackCollection::const_iterator trk = l1TTTracks_.begin(); trk != l1TTTracks_.end() ; ++trk){
    ...
    recTk.vertexEta = trk->getPOCA().eta();
    recTk.vertexPhi = trk->getPOCA().phi();
...
void TrackTriggerStudy::matchStubEGamma(GlobalPoint epos, double ee, TTIStudy::Electron& electron) {
  ...
  for (edmNew::DetSetVector< TTStub<Ref_PixelDigi_> >::const_iterator it  = stubHandle->begin();
	...
	scaledZIntCut = stubEGdZCut_;
      ...
      preSelectedStubs.push_back(stub_ref);
    }
  }
