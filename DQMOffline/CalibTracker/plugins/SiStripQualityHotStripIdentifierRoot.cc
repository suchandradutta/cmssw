...
SiStripBadStrip* SiStripQualityHotStripIdentifierRoot::getNewObject(){
  ...
  if (TotNumberOfEvents>=calibrationthreshold)
	...
	{
	  edm::LogWarning("SiStripQualityHotStripIdentifierRoot")<< "[SiStripQualityHotStripIdentifierRoot::getNewObject] EmptyEvents :: " <<"Mean Number of Cluster is  " << MeanNumberOfCluster << " Calibration not launched";
	  setDoStore(false); // Don't put anything in the sqlite-file!                                                                                                
	}
    }
  else
    {
      edm::LogWarning("SiStripQualityHotStripIdentifierRoot")<< "[SiStripQualityHotStripIdentifierRoot::getNewObject] NotEnoughEvents :: " <<"Total number of events is  " << TotNumberOfEvents << " Calibration not launched";
      setDoStore(false); // Don't put anything in the sqlite-file!
    }
...
void SiStripQualityHotStripIdentifierRoot::bookHistos(){
  ...
  for (; iter!=iterEnd;++iter) {
    ...
    if (!gotNentries && strstr(me_name.c_str(),"TotalNumberOfCluster__T")!=NULL && strstr(me_name.c_str(),"Profile")==NULL ){
      ...
      edm::LogInfo("SiStripQualityHotStripIdentifierRoot")<< "[SiStripQualityHotStripIdentifierRoot::bookHistos] Total Number of Events: " << TotNumberOfEvents << std::endl;
      ...
      edm::LogWarning("SiStripQualityHotStripIdentifierRoot")<< "[SiStripQualityHotStripIdentifierRoot::bookHistos] histogram missing :: " <<"TotalNumberOfCluster histogram missing!!! " ;
    }
