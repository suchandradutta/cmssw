...
#include "Demo/SimHitAnalyzer/plugins/SimHitAnalyzer.h"
  ...
  simHitsEndcapLowTof_ (cfg.getParameter<edm::InputTag> ("simHitsEndcapLowTof"))
{
  ...
  twoDHists_["rhoPhi"] = fs_->make<TH2D> ("rhoPhi", ";x [cm];y [cm]", 500, 0.0, 40.0, 500, 0.0, 20.0);
  twoDHists_["rhoZ"] = fs_->make<TH2D> ("rhoZ", ";z [cm];#rho [cm]", 500, 0.0, 300.0, 500, 0.0, 30.0);
  //twoDHists_["rhoPhi"] = fs_->make<TH2D> ("rhoPhi", ";x [cm];y [cm]", 3000, 0.0, 120.0, 3000, 0.0, 120.0);
  //twoDHists_["rhoZ"] = fs_->make<TH2D> ("rhoZ", ";z [cm];#rho [cm]", 500, 0.0, 300.0, 3000, 0.0, 120.0);//to include OT
